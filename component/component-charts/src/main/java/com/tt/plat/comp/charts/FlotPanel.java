...
public class FlotPanel extends Panel
{
	...
	public FlotPanel(final String id, final IModel<List<Series>> model)
	{
        ...
        final WebMarkupContainer drawAreaWebContainer = new WebMarkupContainer("drawArea");
        drawAreaWebContainer.setOutputMarkupId(true);
        add(drawAreaWebContainer);
		...
		add(new WebComponent("flotScript")
		{
			...
			{
				replaceComponentTagBody(markupStream, openTag, getFlotScript(drawAreaWebContainer.getMarkupId()));
			}
...
// render
	 ...
	 */
	private String getFlotScript(String markId)
	{
		try
		{
...
// "{ lines: { show: true }, points: { show: true }, selection: { mode: \"xy\" }, grid: { hoverable: true, clickable: true }, xaxis: { mode: 'time', min: 1243807200000, max: 1246312800000, timeformat: '%d.%m.%y' }, yaxis: { min: 0, max: 15 }}";
			...
			return String.format(str, markId,strData, strOptions, (showTooltip ? "true" : "false"));
		}
		catch (IOException e)
		{
    ...
    public void setMonthNames(String[] values){
        options.get("xaxis").put("monthNames",values);
    }

    public void setDayNames(String[] values){
        options.get("xaxis").put("dayNames",values);
    }

    public void setGrid(String key,Serializable value){

        options.get("grid").put(key,value);
    }
	...
	public void setAxisMinY(Serializable axisMinY)
	{
		options.get("yaxis").put("min", axisMinY);
	}

	public Serializable getAxisMinY()
	{
		return (Serializable)options.get("yaxis").get("min");
	}

	public void setAxisMaxX(Serializable axisMaxX)
	{
		options.get("xaxis").put("max", axisMaxX);
	}

	public Serializable getAxisMaxX()
	{
		return (Serializable)options.get("xaxis").get("max");
	}

	public void setAxisMaxY(Serializable axisMaxY)
	{
		options.get("yaxis").put("max", axisMaxY);
	}

	public Serializable getAxisMaxY()
	{
		return (Serializable)options.get("yaxis").get("max");
	}
	...
	public void showTooltip(boolean showTooltip)
	{
        ...
        options.get("tooltip").put("tooltip",showTooltip);
	}

    public void setTooltipOptsContent(String content){
        options.get("tooltipOpts").put("content",content);
    }
