 ...
 */
package com.tt.plat.security.shiro.impl.wicket.page.store;
	...
	public void storePages(final SerializedPageWrapper wrapper)
	{
		...
		try
		{
			...
			if (MAX_SIZE != SessionPageStore.DEFAULT_MAX_PAGES)
			{
				...
				if (numToRemove > 0)
				{
					final Iterator<Entry<Integer, SerializedPageWrapper>> iter = pages.entrySet()
						.iterator();
					while (iter.hasNext() && numToRemove > 0)
					{
						final Entry<Integer, SerializedPageWrapper> entry = iter.next();
						iter.remove();
