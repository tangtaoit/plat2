var App = function () {
    ...
    var l = false;
    var b = [];
    var m = {blue: "#54728c", red: "#e25856", green: "#94B86E", purple: "#852b99", grey: "#555555", yellow: "#ffb848"};
    var w = "250px";
    var v = function () {
        ...
        $(window).setBreakpoints({breakpoints: [320, 480, 768, 979, 1200]});
        ...
        if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) && x == false) {
            ...
            if ($("#sidebar").hasClass("sidebar-fixed") || $(window).width() <= 767) {
                if (x && !$("#sidebar").hasClass("sidebar-fixed-responsive")) {
                    ...
                    var y = 7;
                    $("#sidebar-content").slimscroll({height: "100%", wheelStep: y})
                }
            }
        }
    };
    ...
    var a = function () {
        $(".scroller").each(function () {
            $(this).slimScroll({size: "7px", opacity: "0.2", position: "right", height: $(this).attr("data-height"), alwaysVisible: ($(this).attr("data-always-visible") == "1" ? true : false), railVisible: ($(this).attr("data-rail-visible") == "1" ? true : false), disableFadeOut: true})
        })
    };
    var p = function () {
        c();
        ...
        $(".project-switcher #frame").each(function () {
            $(this).slimScrollHorizontal({width: "100%", alwaysVisible: true, color: "#fff", opacity: "0.2", size: "5px"})
        });
        var x = function (z) {
    ...
    return{init: function () {
        ...
        var x = $(x);
        x.block({message: '<img src="./assets/img/ajax-loading.gif" alt="">', centerY: y != undefined ? y : true, css: {top: "10%", border: "none", padding: "2px", backgroundColor: "none"}, overlayCSS: {backgroundColor: "#000", opacity: 0.05, cursor: "wait"}})
    }, unblockUI: function (x) {
        $(x).unblock({onUnblock: function () {
            $(x).removeAttr("style")
        }})
    }}
}();